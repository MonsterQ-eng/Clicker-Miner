...
public class DiamonScript : MonoBehaviour
{
   ...
   public float profit = 200;
    ...
    void Start()
    {
        ...
        isActivediamond = PlayerPrefsX.GetBool("diamondActiv", false);
        if (PlayerPrefs.HasKey("diamondUpgrade"))
        {
            upgradeBool = PlayerPrefsX.GetBoolArray("diamondUpgrade");
        }
    ...
    public void UpgradeButton()
    {
        ...
        if (!upgradeBool[0])
        {
            if (money >= upgradeCost[0])
            {
                ...
                upgradeBool[0] = true;
                PlayerPrefsX.SetBoolArray("diamondUpgrade", upgradeBool);
    ...
    {
        money = PlayerPrefs.GetFloat("money", 0);
        if (money >= gameCont.newMineshaftCost[4])
        {
            money -= gameCont.newMineshaftCost[4];
            PlayerPrefs.SetFloat("money", money);
            gameCont.SaveMineshaft(4);

            isActivediamond = true;
            PlayerPrefsX.SetBool("diamondActiv", isActivediamond);  //Save diamond
            UpdateCoroutine();
